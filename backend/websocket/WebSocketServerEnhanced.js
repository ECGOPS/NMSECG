...
class WebSocketServerEnhanced {
  constructor(server) {
    ...
    // Start health check for Azure App Service
    this.startHealthCheck();
    
    console.log('✅ Enhanced WebSocket server initialized for Azure App Service');
  }
  ...
  async handleMessage(userId, message) {
    ...
    switch (message.type) {
      ...
      case 'delete_chat_message':
        ...
        break;
      case 'ping':
        // Azure optimization: Handle ping messages
  ...
  async handleInitialBroadcastsRequest(userId, message) {
    try {
      ...
      // Get recent broadcast messages (limit to last 20)
      const { resources: messages } = await container.items.query('SELECT * FROM c WHERE c.isActive = true ORDER BY c.createdAt DESC OFFSET 0 LIMIT 20').fetchAll();
  ...
  async handleDeleteBroadcastMessage(userId, message) {
    try {
      ...
      if (!endpoint || endpoint.includes('dev-cosmos-endpoint')) {
        console.log('Development mode: broadcast message deletion not supported');
        return;
      }
      ...
      console.log(`🗑️ Deleted broadcast message ${message.messageId} from database`);
  ...
  async handleDeleteChatMessage(userId, message) {
    try {
      ...
      if (!endpoint || endpoint.includes('dev-cosmos-endpoint')) {
        console.log('Development mode: chat message deletion not supported');
        return;
      }
      ...
      // Delete the message from database
      await container.item(message.messageId, message.messageId).delete();
      console.log(`🗑️ Deleted chat message ${message.messageId} from database`);
...
module.exports = WebSocketServerEnhanced;
