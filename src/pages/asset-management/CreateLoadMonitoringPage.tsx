...
export default function CreateLoadMonitoringPage() {
  ...
  const { regions, districts } = useData(); // Get regions & districts
  const { createRecord, isOnline } = useLoadMonitoringOfflineCRUD(); // Use offline CRUD
  ...
  useEffect(() => {
    ...
    if (user.role === "ashsubt") {
      ...
      if (ashsubtRegions.length > 0) {
        ...
        // Only show districts from the default region, not all allowed regions
        const regionDistricts = districts.filter(d => d.regionId === defaultRegion.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const completeData: Omit<LoadMonitoringData, 'id'> = {
      ...
      ownership: formData.ownership as 'public' | 'private',
    ...
    try {
      const result = await createRecord(completeData);
      if (result.synced) {
        toast.success("Load monitoring record created successfully!");
      } else {
      ...
      toast.error("Failed to create load monitoring record");
    }
  };

  // Problem analysis & recommendations (recomputed on inputs)
  const analysis = useMemo(() => {
    ...
    if (worstLegIndex >= 0 && worstPhase) {
      issues.push(`Highest current ${worstLegIndex >= 0 ? `on Leg ${worstLegIndex+1} `: ''}(${worstPhase} phase: ${worstValue.toFixed(2)} A).`);
    }

    // Phase totals and move plan
    const phaseTotals = feederLegs.reduce((acc, leg) => {
      acc.red += safeNum(leg.redPhaseCurrent); acc.yellow += safeNum(leg.yellowPhaseCurrent); acc.blue += safeNum(leg.bluePhaseCurrent); return acc;
    ...
    const addPlan = (fromKey: 'red'|'yellow'|'blue', toKeys: Array<'red'|'yellow'|'blue'>, out: any[]) => {
      const excess = Math.max(0, phaseTotals[fromKey] - targetPerPhase);
      if (excess <= 0.01) return;
      let remaining = excess; const to: Array<{phase:'Red'|'Yellow'|'Blue'; amountA:number}> = [];
      ...
      out.push({ from: (fromKey.charAt(0).toUpperCase()+fromKey.slice(1)) as 'Red'|'Yellow'|'Blue', to, excessA: Number(excess.toFixed(2)) });
    };
    const movePlan: Array<{ from:'Red'|'Yellow'|'Blue'; to:Array<{phase:'Red'|'Yellow'|'Blue'; amountA:number}>; excessA:number }> = [];
    addPlan('red', ['yellow','blue'], movePlan); addPlan('yellow', ['red','blue'], movePlan); addPlan('blue', ['red','yellow'], movePlan);
    ...
    return {
      ...
      movePlan,
      legPlans,
      recommendation: issues.find(i=>i.includes('overloaded'))
        ? 'Reduce load immediately or consider uprating transformer capacity. Investigate large single-phase loads and redistribute where possible.'
        : (issues.find(i=>i.includes('Severe phase imbalance')) || issues.find(i=>i.includes('Neutral')))
          ? 'Rebalance phases and inspect neutral connections; shift single-phase loads off the heaviest phase.'
          : 'No critical issues detected. Maintain balanced loading and continue monitoring.'
    };
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      required
                      disabled={user?.role === "district_engineer" || user?.role === "district_manager" || user?.role === "technician" || (!formData.regionId && !(user?.role === "ashsubt" || user?.role === "accsubt")) || filteredDistricts.length === 0}
                    >
