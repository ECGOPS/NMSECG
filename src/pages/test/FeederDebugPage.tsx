...
export default function FeederDebugPage() {
  ...
  const testDirectAPI = async () => {
    setIsLoading(true);
    try {
      console.log('[FeederDebug] Testing direct API call to /api/feeders');
      const result = await apiRequest('/api/feeders');
      console.log('[FeederDebug] Direct API result:', result);
      addTestResult('Direct API Call', result);
      toast.success('Direct API test completed');
    } catch (error) {
      console.error('[FeederDebug] Direct API error:', error);
  ...
  const testGetAllFeeders = async () => {
    setIsLoading(true);
    try {
      console.log('[FeederDebug] Testing getAllFeeders()');
      const result = await feederService.getAllFeeders();
      console.log('[FeederDebug] getAllFeeders result:', result);
      addTestResult('getAllFeeders()', result);
      toast.success('getAllFeeders test completed');
    } catch (error) {
      console.error('[FeederDebug] getAllFeeders error:', error);
  ...
  const testGetFeedersByRegion = async () => {
    ...
    setIsLoading(true);
    try {
      console.log('[FeederDebug] Testing getFeedersByRegion() with region:', selectedRegion);
      const result = await feederService.getFeedersByRegion(selectedRegion);
      console.log('[FeederDebug] getFeedersByRegion result:', result);
      addTestResult('getFeedersByRegion()', result);
      toast.success('getFeedersByRegion test completed');
    } catch (error) {
      console.error('[FeederDebug] getFeedersByRegion error:', error);
  ...
  const testDirectRegionAPI = async () => {
    ...
    setIsLoading(true);
    try {
      console.log('[FeederDebug] Testing direct API call with regionId:', selectedRegion);
      const result = await apiRequest(`/api/feeders?regionId=${selectedRegion}`);
      console.log('[FeederDebug] Direct region API result:', result);
      addTestResult('Direct Region API Call', result);
      toast.success('Direct region API test completed');
    } catch (error) {
      console.error('[FeederDebug] Direct region API error:', error);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Feeder Loading Debug</CardTitle>
          <CardDescription>
            Debug feeder loading issues on different pages
          </CardDescription>
        </CardHeader>
